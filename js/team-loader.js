...
(function () {
  var SECTION_SEL = '.team-one-section';
  var LIST_SEL = '.team-one-list';
  ...
  function renderCard(m) {
    ...
    var linkedin = m.linkedin
      ? '<a href="' + esc(m.linkedin) + '" target="_blank" rel="noopener" class="team-one-social-link" aria-label="LinkedIn">' +
        ...
        '</a>'
      : '';
    var deptHtml = m.department
      ? '<div class="team-one-department">' + esc(m.department) + '</div>'
      : '';
  ...
  async function load() {
    ...
    if (!section) return;
    var list = section.querySelector(LIST_SEL) || section;
    try {
      var firebaseConfig = {
